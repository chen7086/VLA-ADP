...
def get_vla(cfg: Any) -> torch.nn.Module:
    ...
    try:
        # Debug which implementation is actually used (local vs HF Hub remote)
        is_hub = model_is_on_hf_hub(cfg.pretrained_checkpoint)
        print(
            f"[OPENVLA_DEBUG] model_is_on_hf_hub={is_hub}; class={vla.__class__.__name__}; module={vla.__class__.__module__}"
        )
    except Exception:
        pass
    ...
    with torch.inference_mode():
        ...
        try:
            if getattr(cfg, "qk_keep_enabled", False):
                layer = int(getattr(cfg, "qk_layer", 0))
                ratio = float(getattr(cfg, "qk_keep_ratio", 0.75))
                split = getattr(cfg, "qk_keep_split", None)
                debug = bool(getattr(cfg, "qk_debug", False))
